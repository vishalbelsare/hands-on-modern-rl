---
title: 14.1 DPO Objective and Derivation
---
...
A preference dataset only defines the training signal. To see where DPO sits in the lifecycle of a large model, we need to lay out the full training pipeline. Modern LLM training is usually described as two big phases: **pre-training** and **post-training**. Pre-training gives the model language ability and world knowledge, but the resulting base model is mostly a continuation engine that cannot hold a conversation. Post-training adapts that base model into a usable assistant, and is often further split into supervised fine-tuning (SFT) and alignment via RL-style objectives (RL / Alignment). As a pipeline [^2][^4]:

$$\text{Pre-training} \;\xrightarrow{\text{Base Model}}\; \underbrace{\text{SFT} + \text{RL}}_{\text{Post-Training}} \;\xrightarrow{\text{Chat Model}}$$

We'll explain each stage, then derive the DPO objective.
...
Experiment 1 in Section 14.4 will run [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py), and the `preference_data.json` it loads is in exactly this format. The data loading part parses the JSON file into `prompt`, `chosen`, and `rejected` fields, corresponding to $x$, $y_w$, and $y_l$ in the notation:
...
On a typical GPU, this whole run can finish in a few minutes for a small model. The [supplementary derivation](./dpo-theory-and-family#deriving-dpo-in-three-steps) develops the loss step by step. The following diagram summarizes the evolution across these three stages:
...
Reviewing the entire Post-Training pipeline: SFT solves the problem of "can the model converse," while the alignment stage (RL / DPO) solves "are the responses good." DPO sits at the final step of this pipeline. The next question is: what exactly is DPO optimizing? How is its loss function derived from preference data? The following section develops this derivation step by step.
...
This means that during training, two large models must be loaded into GPU memory simultaneously: one policy model generating responses, and one reward model scoring them. Memory usage doubles, and the two models depend on each other, making training stability worse [^5].

**DPO (Direct Preference Optimization)** [^3] asks a key question: can we skip training a reward model and directly optimize the language model with preference data?
...
Looking again at the code in [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py), `DPOTrainer` receives only a `model` parameter at initialization:
...
The next section opens DPO's training log and uses the derivation from this section to interpret metrics like Loss, Reward Margin, and Accuracy; Section 14.4 then runs the full alignment experiment hands-on.
...
[^2]: Ouyang, L., et al. (2022). Training language models to follow instructions with human feedback. _NeurIPS 2022_. [arXiv:2203.02155](https://arxiv.org/abs/2203.02155)
...
[^4]: Touvron, H., et al. (2023). LLaMA: Open and Efficient Foundation Language Models. _arXiv preprint_. [arXiv:2302.13971](https://arxiv.org/abs/2302.13971)

[^5]: Christiano, P. F., et al. (2017). Deep reinforcement learning from human preferences. _Advances in Neural Information Processing Systems_, 30. [arXiv:1706.03741](https://arxiv.org/abs/1706.03741)
