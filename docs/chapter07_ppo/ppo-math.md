...
如果手里有新策略采出来的动作，直接对这些 $f(a)$ 求平均即可。但现在没有新策略样本，只有旧策略 $\pi_{\text{old}}$ 采出来的动作。直接平均会有偏差，因为旧策略和新策略对同一个动作的偏好不同。
...
策略更新时，$f(a)$ 对应这个动作的优势 $A_t$。优势告诉我们：这个动作相对于当前状态的平均水平，是更好还是更差。
