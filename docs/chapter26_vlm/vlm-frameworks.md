...
| 第 14 章：Agentic RL | 文本 + 工具轨迹 | 工具调用 / token | 结果奖励 + 过程奖励     | PPO/GRPO  |
| 第 15 章：VLM RL     | 图像 + Token    | 离散（token）    | 规则 + 模型 + grounding | GRPO      |
...
把本章的 VLM RL 和[第 10 章](../chapter22_agentic/intro)的 Agent RL 结合，会面临三个额外的挑战：
...
**3. 跨模态 Credit Assignment。** 在一条 10 轮的轨迹中，第 2 轮的视觉理解错误可能导致第 5 轮的工具调用失败。这比纯文本 Agent 的 credit assignment 更难，因为跨模态的错误传播链更长、更隐晦。[第 10 章](../chapter22_agentic/multi-turn-rl)讨论的 ORM vs PRM 取舍在这里更加突出。
...
2. **再练工具调用**：用[第 10 章的工具调用 RL](../chapter22_agentic/tool-use-and-trajectory)训练基本的工具使用模式
