...
def compute_gae(
    ...
    Args:
        ...
        next_values: V(s') after each transition. 0 when terminated, the estimate is kept when truncated
        episode_ends: whether a reset happened after each step, used to stop GAE propagating across episodes
...
def collect_trajectories(
    ...
    Collected data:
        ...
        - rewards: rewards
        - next_values: next-state value at each step
        - episode_ends: whether a reset occurred after that step
    ...
    for step in range(n_steps):
        ...
        # V(s')=0 on a genuine termination; both time truncation and the rollout tail must bootstrap.
        if done:
            next_value = 0.0
        else:
            with torch.no_grad():
