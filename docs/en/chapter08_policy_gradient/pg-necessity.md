...
### The Number of Actions Explodes
...
Discretization does not remove the difficulty. If each of six joints is restricted to 100 torque values, the combined action set contains

$$
N=100^6=10^{12}
$$

actions. At one microsecond per evaluation, choosing a single action would require roughly

$$
10^{12}\times10^{-6}\text{s}=10^6\text{s}\approx11.6\text{ days}.
$$

A policy network instead performs one forward pass and outputs the parameters of the action distribution. The number of grid points never enters the computation.

### Language Generation Needs a Distribution
...
This is the central idea of Chapter 3's discussion of [policy, value, and return](../chapter03_mdp/policy-value): define a policy objective $J(\theta)$ and optimize the parameters $\theta$ to maximize it.
...
### What the Policy Network Produces

For discrete actions, $\pi_\theta(a\mid s)$ outputs one probability per action. A CartPole policy might assign

$$
\pi_\theta(\text{left}\mid s)=0.3,
\qquad
\pi_\theta(\text{right}\mid s)=0.7.
$$

The action is sampled from this distribution. For continuous control, the network can instead output the mean and standard deviation of a Gaussian distribution. A six-joint robot policy produces six means and six standard deviations, then samples one torque vector in a single forward pass.

### Compared with DQN

DQN produces action values and applies $\arg\max$; the policy network produces a distribution and samples from it. The first route needs an external exploration rule such as $\varepsilon$-greedy. The second route represents exploration directly through its probabilities.
...
### Numerical Comparison on the Same States

Consider three states $\{s_1,s_2,s_3\}$ and two actions $\{a_1,a_2\}$. A trained DQN might produce

| State | $Q(s,a_1)$ | $Q(s,a_2)$ | Greedy action |
| ----- | ---------- | ---------- | ------------- |
| $s_1$ | $1.5$      | $2.3$      | $a_2$         |
| $s_2$ | $0.8$      | $-0.4$     | $a_1$         |
| $s_3$ | $3.1$      | $2.9$      | $a_1$         |

The $Q$ values at $s_3$ are close, but a greedy policy still chooses $a_1$ every time. Adding $\varepsilon$-greedy exploration assigns the same exploration rate to $s_3$ as to states where one action is clearly better.

A policy network can represent the uncertainty directly:
...
At $s_3$, both actions remain plausible, so the policy continues to sample both. At $s_1$ and $s_2$, the distribution is sharper. The amount of exploration now depends on the state instead of a single global $\varepsilon$.
