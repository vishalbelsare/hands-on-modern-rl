...
前两章我们做了两件事：第 1 章让一根木棍学会了直立不倒，第 2 章让一个小模型学会了在被拒绝时保持礼貌。两次都成功了——CartPole 拿到了高分，DPO 产出了更友好的回复。但有一个问题一直被绕过：**这些算法到底是怎么"学会"的？**

PPO 为什么能在 20000 步之后突然"开窍"？DPO 的损失函数为什么能把偏好信号注入模型参数？答案藏在同一个地方——一个叫 MDP 的数学框架里。但在拆开黑盒之前，让我们先通过一个简单的游戏，亲身体验 RL 最核心的决策困境。
...
> 你先拉哪台？拉了几次之后，你还继续两边试，还是选一台专攻？

如果你想到的是"先两边都试试，搞清楚哪个更赚，然后锁定它"——这就是 RL 最核心的策略思想：**探索与利用（Exploration vs. Exploitation）**。"试试"是探索（exploration），"锁定"是利用（exploitation）。整个 RL 的学习过程，本质上就是在这两者之间找平衡。

这个困境无处不在。选餐厅：去吃过的老店（利用），还是试一家新店（探索）？看电影：看喜欢的导演的新片（利用），还是试试陌生类型（探索）？RL 把这个日常决策变成了数学问题。

## 三个策略，三种结局

为了使讨论更具体，假设一个上帝视角的事实：A 台出奖率 60%，B 台只有 40%。但玩家不知道——它只能通过尝试来发现。

### 策略 1：闭眼随机（50/50）

每次抛个硬币决定拉哪台，A 和 B 各 50%。

跑 100 轮，总回报在 0 附近波动。因为你一半时间拉了 A（期望每次赚 0.2），一半时间拉了 B（期望每次亏 0.2），一平均就是 0：

$$\mathbb{E}[R] = 0.5 \times 0.2 + 0.5 \times (-0.2) = 0$$

### 策略 2：永远拉 A（如果知道 A 更好）

如果你知道 A 台出奖率更高，那当然永远拉 A：

$$\mathbb{E}[R] = 0.6 \times (+1) + 0.4 \times (-1) = 0.2$$

每轮平均赚 0.2，100 轮下来净赚约 20。但现实中你不知道 A 更好——你需要通过尝试来发现。
...
这段代码已经是一个完整的 RL 环境。`pull()` 接收一个动作（拉 A 或拉 B），返回一个奖励（+1 或 -1）。它遵循和 Gymnasium 完全一致的接口模式——只不过 CartPole 有 4 维连续状态和左右推两个动作，这里只有一个动作选择。

你可能会注意到一个区别：这个环境没有"状态"——不管你上一步拉了哪台机器，这一步面对的情况一模一样。这就是老虎机的特点：它是一个**单状态 MDP**。后面我们会看到 CartPole 和 LLM 就不是这样了——它们的状态会随着动作而改变。

## 跑一把看看

以下代码演示了三个策略在同一个环境下的表现：
...
答案是我们已经用了好几次的概念——**期望回报**。它是"如果你玩无数次的平均得分"：
...
退后一步，看看刚才发生了什么。不管你是拉老虎机、控制 CartPole、还是训练大模型，RL 的交互模式都遵循同一个循环：
...
智能体选择动作，环境给出奖励和新状态，循环往复。智能体的目标就是让累积奖励尽可能大。
...
要回答这些问题，我们需要一套更完整的数学语言。下一节将把"状态、动作、奖励"这些直观概念提炼成精确的数学框架。[MDP 五元组、折扣回报与策略](./mdp)
