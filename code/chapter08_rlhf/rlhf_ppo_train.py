...
This script demonstrates stage three of the RLHF pipeline — PPO alignment training.
It covers:
  1. Loading the SFT model as the policy model (Actor)
  2. Scoring generated responses with the reward model
  3. The PPO training loop: generate → score → compute advantages → clipped update
  4. Tracking reward, KL divergence, response length, and other metrics
  5. Comparing response quality before and after alignment

Note: this is a simplified/simulated version. Full RLHF-PPO training usually requires:
  - A large cluster (tens to hundreds of GPUs)
  - Hundreds of thousands of preference pairs
  - A sophisticated distributed training framework
  This script aims to convey PPO's core algorithmic flow within RLHF.
"""
...
def build_response_mask(attention_mask, prompt_length):
    """Build a response-token mask aligned with the shifted labels."""
...
def compute_log_probs(model, input_ids, attention_mask, prompt_length):
    ...
    # The prompt only provides conditioning and is not a policy action; count response tokens only.
...
class SimpleRewardModel:
    ...
    In real RLHF, the reward model is a deep network trained on preference pairs.
    Here we use a rule-based scoring function to mimic its behavior, so the PPO
    alignment flow can be demonstrated quickly on a single machine.
    ...
    def __init__(self, tokenizer, backbone_model=None, value_head_path=None):
        ...
        if backbone_model is not None:
            ...
            if value_head_path and os.path.exists(value_head_path):
                ...
                raise FileNotFoundError("Reward model is missing trained value-head parameters")
...
class PPOTrainer:
    ...
    where r(θ) = π_θ(a|s) / π_old(a|s) is the probability ratio between the new and old policies.
    ...
    def train_step(self, prompts):
        ...
        for prompt in prompts:
            ...
            # ---- Step 3: record the old policy's response log probabilities ----
            with torch.no_grad():
        ...
        # ---- Step 6: run multiple PPO clipped updates over the same batch of rollouts ----
...
def main():
    ...
    print("  [2] RM (reward model training)     → output/rm_results/")
