...
::: warning 分层 RL 的实际困境
分层 RL 听起来优雅，但工业落地少。原因：(1) 层次结构本身是强归纳偏置，错配会反向伤害性能；(2) 高层与底层耦合训练易陷入"互相欺骗"局部解——Manager 给无意义方向，Worker 学着忽略它；(3) LLM 时代的"分层"已经从神经网络架构转移到 prompt 层（plan-then-act、ReAct），更易调试。但思想仍深刻影响 agentic RL（[第 25 章](../chapter22_agentic/tool-use-and-trajectory)）和 [第 38 章 多智能体](../chapter32_selfplay/llm-multi-agent-rl/)。
:::
...
[第 11 章 Dreamer V3](../chapter11_continuous_control/intro#_12-7-dreamer-v3-世界模型的新世代) 已经展示了"在世界模型里训练 actor-critic"的可行性：先用真实数据训一个 RSSM 世界模型，再在想象轨迹里优化策略。Dreamer 的世界模型仍是任务相关的（在某个 Atari 游戏或 MuJoCo 环境上训）。
...
学到的 $z_t$ 可作为 RL 的动作空间，使得在 Genie 生成的环境中训练的 agent 能迁移到真实控制任务。这是 model-based RL（[第 11 章](../chapter11_continuous_control/intro#_12-5-model-based-rl-学习环境模型)）+ 视频生成模型 + 探索-利用理论的交汇点。
...
这三条路线在 LLM 时代再次汇合：agentic RL 的工具调用本质是 option、多 agent 协作本质是 CTDE、LLM 内嵌的世界知识本质是 Genie 式生成环境。下一章 [第 16 章 RLHF 训练流水线](../chapter15_rlhf/intro) 进入大模型对齐主线，那里的"环境"是 LLM 自身，但本章的探索、多智能体、分层思想将贯穿后续所有 LLM RL 章节。
