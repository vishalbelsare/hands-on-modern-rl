...
def print_fix_summary():
    ...
    fixes = [
        {
            "failure": "Loss explosion (loss suddenly becomes NaN/Inf)",
            "symptom": "The loss curve suddenly spikes to an astronomical value",
            "cause": "Learning rate too high",
            "fix": "Lower the learning rate (recommended 1e-4 ~ 1e-3), use a learning rate scheduler",
            "code": "optimizer = Adam(params, lr=1e-3)  # not 1.0",
        },
        {
            "failure": "Gradient explosion (gradient norm keeps growing)",
            "symptom": "Gradient norm > 1000 and not converging",
            "cause": "Missing gradient clipping",
            "fix": "Add gradient clipping via clip_grad_norm_",
            "code": "torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=10)",
        },
        {
            "failure": "Never converges (reward stays low and noisy)",
            "symptom": "No upward trend in reward even after hundreds of episodes",
            "cause": "Exploration rate epsilon does not decay",
            "fix": "Use an exponential or linear epsilon decay schedule",
            "code": "epsilon = max(0.01, epsilon * 0.995)  # decay every episode",
        },
        {
            "failure": "Policy degeneration (only picks one action)",
            "symptom": "Extremely skewed action distribution (e.g. 99:1)",
            "cause": "Epsilon decays too fast or its initial value is too low",
            "fix": "Slow down epsilon decay, raise the final epsilon floor",
            "code": "epsilon = max(0.05, epsilon * 0.999)  # slower decay",
        },
    ]

    for i, fix in enumerate(fixes, 1):
        print(f"\n  Failure {i}: {fix['failure']}")
        print(f"    Symptom: {fix['symptom']}")
