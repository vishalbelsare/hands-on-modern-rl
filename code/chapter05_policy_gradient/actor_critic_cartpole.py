...
def compute_advantage(reward, value, next_value, gamma=0.99, terminated=False):
    ...
    Args:
        ...
        terminated: whether the MDP terminal state was reached. Time truncation still bootstraps from V(s')
    Returns:
    ...
    if terminated:
        # At an MDP terminal state there is no next-state value, so the target = r_t
...
def compute_actor_critic_losses(
    ...
    """Compute the one-step Actor-Critic loss, blocking gradients from the policy loss into the value estimate."""
...
def train():
    ...
    for episode in range(num_episodes):
        ...
        while not (done or truncated):
            ...
            # Only terminated zeroes the next-state value; truncated still bootstraps from V(s').
