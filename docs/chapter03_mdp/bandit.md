...
接下来真正影响策略回报的是：**总共有多大概率会误判**。

误判条件很简单：探索 10 次以后，A 的中奖次数少于 B 的中奖次数，也就是

$$n_A < n_B$$

计算方式是：把所有可能的 $(n_A,n_B)$ 都列出来。$n_A$ 可以是 0 到 10，$n_B$ 也可以是 0 到 10。只要满足 $n_A<n_B$，就说明这次探索会把 B 看成更好。

例如：

- A 中 4 次，B 中 5 次：误判。
- A 中 3 次，B 中 6 次：误判。
- A 中 5 次，B 中 5 次：不算 B 更好。
- A 中 6 次，B 中 4 次：不会误判。

每一种组合都有一个概率。因为 A 的 10 次探索和 B 的 10 次探索是分开做的，所以某个组合的概率可以相乘：

$$P(n_A=i, n_B=j)=P_A(n_A=i)\times P_B(n_B=j)$$

最后把所有 $i<j$ 的组合概率加起来：

$$P(\text{误判})=\sum_{i<j}P_A(n_A=i)P_B(n_B=j)\approx 18.5\%$$

也就是说，大约每 5 次完整实验里，就有 1 次会因为短期样本误判。
